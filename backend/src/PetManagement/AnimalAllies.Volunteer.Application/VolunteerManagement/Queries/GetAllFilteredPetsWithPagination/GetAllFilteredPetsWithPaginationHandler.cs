...
public class
    GetAllFilteredPetsWithPaginationHandler : IQueryHandler<PagedList<PetDto>, GetAllFilteredPetsWithPaginationQuery>
{
    private readonly ISqlConnectionFactory _sqlConnectionFactory;
    private readonly ILogger<GetAllFilteredPetsWithPaginationHandler> _logger;
    private readonly IValidator<GetAllFilteredPetsWithPaginationQuery> _validator;

    public GetAllFilteredPetsWithPaginationHandler(
        [FromKeyedServices(Constraints.Context.PetManagement)]
        ISqlConnectionFactory sqlConnectionFactory,
        ILogger<GetAllFilteredPetsWithPaginationHandler> logger,
        IValidator<GetAllFilteredPetsWithPaginationQuery> validator)
    {
    ...
    public async Task<Result<PagedList<PetDto>>> Handle(
        GetAllFilteredPetsWithPaginationQuery query,
        CancellationToken cancellationToken = default)
    {
        ...
        var totalCountSql =
            new StringBuilder(
                "select count(*) from volunteers.pets where is_deleted = false");
        ...
        var totalCount =
            await connection.ExecuteScalarAsync<int>(totalCountSql.ToString());
    ...
    private static void FilterByValue(ref bool hasWhereClause, GetAllFilteredPetsWithPaginationQuery query,
        StringBuilder sql)
    {
        switch (query)
        {
        ...
        if (query.IsCastrated is not null)
            sql.ApplyFilterByEqualsValue(ref hasWhereClause, "is_spayed_neutered", query.IsCastrated);
